		...
		if (!empty($arguments['where']) && is_array($arguments['where'])) {
			foreach ($arguments['where'] as $clause) {
				...
				if (!in_array($col, $schema, true)) {
					return ToolResult::error('Unknown WHERE column: ' . $col);
				}
				if (!in_array($op, self::ALLOWED_OPS, true)) {
					return ToolResult::error('Disallowed WHERE op: ' . $op);
				}
				...
				if ($op === 'IS NULL' || $op === 'IS NOT NULL') {
					...
					if ($values === []) {
						return ToolResult::error('IN requires a non-empty value array.');
					}
					...
					if ($value === null) {
						return ToolResult::error('Operator ' . $op . ' requires value.');
					}
